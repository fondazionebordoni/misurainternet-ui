...
class App extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      par: (
        <p>
          Interfaccia web per il monitoraggio della qualità degli accessi ad
          Internet da postazione fissa realizzato da AGCOM in collaborazione con
          la Fondazione Ugo Bordoni ed il supporto dell’Istituto Superiore delle
          Comunicazioni.
        </p>
      ),
  ...
  displayNotification(error_code, message) {
    ...
    listelements.push(
      <li className="list-group-item">
        <Notifica
          text={
            h + ":" + m + "-" + message + "(codice errore: " + error_code + ")"
          }
        />
      </li>
    );
    ...
    if (error) {
      this.setState({
        par:
          ...
          " fallito: " +
          error +
          ". Nuovo tentativo fra pochi secondi..."
      });
    } else {
      switch (test_type) {
        case "ping": {
  ...
  displayTestN(n_test, n_tot, retry) {
    if (retry === "True") {
      this.setState({
        par:
          "Misura ripresa dopo l'interruzione. Test " +
          n_test +
          " di " +
          n_tot +
          "."
  ...
  displayWaitView(serial, message, seconds) {
    ...
    if (serial && serial.length > 0) {
      ...
      if (this.state.isNeMeSysRunning) {
        ...
        $.ajax(settingsNumMeasures).done(
          function(response) {
            this.setState({ misCorrenti: response.numMeasures });
          }.bind(this)
        );

        $.ajax(settingsLicenceInfo).done(
          function(response) {
  ...
  displayEndView() {
    if (!this.state.isNeMeSysRunning) {
      ...
      this.setState({
        par: (
          <p>
            ...
            Puoi effettuare una nuova misurazione cliccando sul tasto START.
            Qualora volessi riprendere ad effettuare le misurazioni con Nemesys,{" "}
            <a href="/">clicca qui</a>
          </p>
        ),
        unitMeasure: "",
      ...
      this.setState({
        par:
          "Le misurazioni sono state completate. Accedi all'area personale per scaricare il certificato."
      });
    }
  }
      ...
      case 1234: //Errore connessione websocket
        this.setState({
          ...
          par: (
            <p>
              <b>
                Nemesys non è al momento operativo oppure non è installato.{" "}
              </b>
              Puoi effettuare una misurazione tramite Misurainternet Speedtest
              premendo sul tasto START. In alternativa{" "}
              <a href="/">clicca qui</a> per riprendere le misurazioni con
              Nemesys.
            </p>
          ),
          dataPing: null,
      ...
      case 1238: //Errore che viene notificato quando, durante il test di download e upload, la prequalifica restituisce sempre zero (assenza di connessione)
        this.setState({
          ...
          par: (
            <p>
              <b>
                Errore! Assenza di connessione ad internet. Controlla la tua
  ...
  render() {
    return (
      <div>
        <Intestazione
          hdr={this.state.hdr}
          licenceInfo={this.state.licenceInfo}
          par={this.state.par}
        />
        {this.state.isNeMeSysRunning && (
        ...
        {(this.state.isNeMeSysRunning ||
          (this.state.dataPing &&
            this.state.dataDownload &&
            this.state.dataUpload)) && (
          <Riepilogo
            isNeMeSysRunning={this.state.isNeMeSysRunning}
